"""
Database operations for skill domains.
"""
...
def list_records(domain: str, item_type: str = "all", verbose: bool = False,
    ...
    Args:
        domain: Domain name
        item_type: Type to list ('product', 'customer', 'operator', 'all')
        verbose: If True, return full record data
        config: Optional domain configuration

    Returns:
        Dictionary mapping type names to lists of records
    """
    if config is None:
        config = get_domain_config(domain)
...
def get_record(domain: str, record_type: str, name_or_id: str,
    ...
    Args:
        domain: Domain name
        record_type: Type of record ('product', 'customer', 'operator')
        name_or_id: Record name or ID to search for
        config: Optional domain configuration

    Returns:
        Tuple of (record_data, record_dir) or (None, None) if not found
    """
    if config is None:
        config = get_domain_config(domain)
...
def update_record(domain: str, record_type: str, name_or_id: str,
    ...
    Args:
        ...
        name_or_id: Record name or ID to update
        updates: Dictionary of fields to update
        config: Optional domain configuration

    Returns:
        Path to updated record directory, or None if not found
    """
    if config is None:
        config = get_domain_config(domain)
    ...
    # Update fields (preserve metadata)
...
def delete_record(domain: str, record_type: str, name_or_id: str,
    ...
    Args:
        domain: Domain name
        record_type: Type of record
        name_or_id: Record name or ID to delete
        config: Optional domain configuration
...
def print_results(results: dict, config: Optional[dict] = None, verbose: bool = False):
    ...
    Args:
        results: Dictionary from list_records
        config: Domain configuration (for labels)
        verbose: If True, print full details
    """
    # Default labels if no config provided
    labels = {
        'product': '产品',
        'customer': '客户角色',
        'operator': '策略'
