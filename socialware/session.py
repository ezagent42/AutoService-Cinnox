...
Provides session ID generation, session lifecycle, and session persistence.
Domain-specific prefixes (DOMAIN_PREFIXES) live in L2 (autoservice/domain_session.py).
"""
...
from socialware.core import ensure_dir
...
def init_session(domain: str, config: Optional[dict] = None,
                 prefix: Optional[str] = None) -> tuple[str, Path]:
    ...
    Args:
        ...
        prefix: Optional domain prefix override (e.g. 'cs', 'mk').
            If None, uses first 2 chars of domain.
    ...
    session_id = generate_session_id(domain, config=config, prefix=prefix)
...
def generate_session_id(
    ...
    Format: {prefix}_{YYYYMMDD}_{seq}_{claude_session_id}

    Args:
        domain: Domain name
        claude_session_id: Claude Code session ID. If None, auto-detected.
        config: Optional domain configuration
        prefix: Domain prefix (e.g. 'cs', 'mk'). If None, uses domain[:2].
